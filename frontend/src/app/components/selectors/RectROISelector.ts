...
export class RectROISelector implements Selector<ROISelection2D> {
	...
	private translationMatrix: Array<number> = new Array<number>(6);
	...
	constructor(canvas: HTMLCanvasElement) {
		...
		this.translationMatrix = [1, 0, 0, 1, 0, 0];
	}
	...
	public onMouseDown(eventPoint: SelectionPoint): void {
		console.log('RectROISelector | startMouseSelection | event: ', event);
		const selectionStartX = (eventPoint.positionX) - this.canvasPosition.left;
		const selectionStartY = (eventPoint.positionY) - this.canvasPosition.top;
	...
	public onMouseMove(eventPoint: SelectionPoint): void {
		if (this.mouseDrag && this.selectedArea) {
			console.log('RectROISelector | drawSelectionRectangle | onmousemove clienXY: ', eventPoint.positionX, eventPoint.positionY);
			this.updateSelection(eventPoint);
	...
	public updateSelection(eventPoint: SelectionPoint): void {
		console.log('RectROISelector | updateSelection | event: ', event);

		if (this.selectedArea) {
			const newWidth = (eventPoint.positionX - this.canvasPosition.left) - this.selectedArea.positionX;
			const newHeight = (eventPoint.positionY - this.canvasPosition.top) - this.selectedArea.positionY;
	...
	public onMouseUp(eventPoint: SelectionPoint): void {
		if (this.mouseDrag) {
	...
	getCurrentSelection(): ROISelection2D {
	...
	updateTranslationMatrix(weights: Array<number>): void {
		this.translationMatrix = weights;
	}
}
